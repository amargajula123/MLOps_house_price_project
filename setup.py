...
packages=find_packages(),  #   it returns packages(folder) where ever __init__.py is having which actually developed by our self
install_requires=get_requirements_list() # it returns all the libaries which is predefined 
)
